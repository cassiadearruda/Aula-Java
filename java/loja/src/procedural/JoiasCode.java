package procedural;
	...
	public static void main(String [] args) {
		...
		// VARIÁVEIS
		final int TAM = 10;
		String produto[] = { "PULSEIRA ICY", "ANEL TOPÁZIO", "COlAR SAFIRA", "PULSEIRA LIZ", "ANEL CRISTAL","BRINCO JASPE", "PINGENTE LUA", "COLAR AMÉLIA", "ANEL CRISTAL", "BRINCO ÁGATA" };
		...
		if (op == 'S') {
			...
			do {
				...
				for (int x = 0; x < 10; x++) {
					if (auxCod.equals(codigo[x])) {
						...
						if (contEstoque[x] > estoque[x]) {
							...
							for (x = 0; x < 10; x++) {
								if (auxQntd[x] > 0) {
									System.out.print(" " + codigo[x] + "\t      " + produto[x] + "\t  R$" + df.format(valorCompra[x]) + "\t       " + auxQntd[x]);
								}
							}
				...
				if (op == 'S') {
					...
					for (int x = 0; x < 10; x++) {
						System.out.print("  " + codigo[x] + "       " + produto[x] + "\t   " + valores[x] + "\t     " + estoque[x]);
					}
			...
			if (op == 'N') {
				...
				while (op2 == 'S') {
					...
					linha();
					System.out.print("CÓDIGO\t        PRODUTO\t\t   PREÇOS\t  QNTDE");
